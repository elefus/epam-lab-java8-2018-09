...
class Exercise3 {
    ...
    private static class LazyMapHelper<T, R> {
        ...
        private Function<T, R> mapperFunction;

        LazyMapHelper(List <T> source, Function <T, R> mapperFunction) {
            this.source = source;
        ...
        public List<R> force() {
            ...
            source.forEach(element -> newList.add(mapperFunction.apply(element)));
            return newList;
        }

        public <R2> LazyMapHelper<T, R2> map(Function<R, R2> mapping) {
            return new LazyMapHelper <>(source, mapperFunction.andThen(mapping));
        }
    }
