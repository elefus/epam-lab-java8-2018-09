...
class Exercise2 {
    ...
    private static class MapHelper<T> {
        ...
        public <R> MapHelper<R> map(Function<T, R> mapping) {
            List <R> listSource = new ArrayList<>();
            source.forEach(el -> listSource.add(mapping.apply(el)));
            return from(listSource);
        }
        ...
        public <R> MapHelper<R> flatMap(Function<T, List<R>> flatMapping) {
            List <R> listSource = new ArrayList<>();
            source.forEach(el -> listSource.addAll(flatMapping.apply(el)));
            return from(listSource);
        }
    }
